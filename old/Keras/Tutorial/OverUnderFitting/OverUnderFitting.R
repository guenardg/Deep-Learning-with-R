...
##
## Now, let’s plot the loss curves for the 3 models. The smaller
## network begins overfitting a litle later than the baseline model
## and its performance degrades much more slowly once it starts
## overfitting. Notice that the larger network begins overfitting
## almost right away, after just one epoch, and overfits much more
## severely. The more capacity the network has, the quicker it will be
## able to model the training data (resulting in a low training loss),
## but the more susceptible it is to overfitting (resulting in a large
...
##
## You may be familiar with Occam’s Razor principle: given two
## explanations for something, the explanation most likely to be
## correct is the “simplest” one, the one that makes the least amount
## of assumptions. This also applies to the models learned by neural
## networks: given some training data and a network architecture,
## there are multiple sets of weights values (multiple models) that
## could explain the data, and simpler models are less likely to
## overfit than complex ones.
##
## A “simple model” in this context is a model where the distribution
## of parameter values has less entropy (or a model with fewer
## parameters altogether, as we saw in the section above). Thus a
## common way to mitigate overfitting is to put constraints on the
## complexity of a network by forcing its weights to only take on
## small values, which makes the distribution of weight values more
## “regular”. This is called “weight regularization”, and it is done
## by adding to the loss function of the network a cost associated
## with having large weights. This cost comes in two flavors:
##
##   > L1 regularization, where the cost added is proportional to the
##     absolute value of the weights coefficients (i.e. to what is
##     called the “L1 norm” of the weights).
##
##   > L2 regularization, where the cost added is proportional to the
##     square of the value of the weights coefficients (i.e. to what
##     is called the “L2 norm” of the weights). L2 regularization is
##     also called weight decay in the context of neural
##     networks. Don’t let the different name confuse you: weight
##     decay is mathematically the exact same as L2 regularization.
##
## In Keras, weight regularization is added by passing weight
## regularizer instances to layers. Let’s add L2 weight regularization
## to the baseline model now.
##
l2_model <- 
    keras_model_sequential() %>%
    layer_dense(units = 16, activation = "relu", input_shape = 10000,
...
## layer will add 0.001 * weight_coefficient_value to the total loss
## of the network. Note that because this penalty is only added at
## training time, the loss for this network will be much higher at
...
## regularization techniques for neural networks, developed by Hinton
## and his students at the University of Toronto. Dropout, applied to
## a layer, consists of randomly “dropping out” (i.e. set to zero) a
## number of output features of the layer during training. Let’s say a
## given layer would normally have returned a vector [0.2, 0.5, 1.3,
## 0.8, 1.1] for a given input sample during training; after applying
## dropout, this vector will have a few zero entries distributed at
## random, e.g. [0, 0.5, 1.3, 0, 1.1]. The “dropout rate” is the
## fraction of the features that are being zeroed-out; it is usually
## set between 0.2 and 0.5. At test time, no units are dropped out,
## and instead the layer’s output values are scaled down by a factor
## equal to the dropout rate, so as to balance for the fact that more
## units are active than at training time.
##
...
##
## Let’s add two dropout layers in our IMDB network to see how well
## they do at reducing overfitting:
dropout_model <- 
    keras_model_sequential() %>%
    layer_dense(units = 16, activation = "relu", input_shape = 10000) %>%
    layer_dropout(0.6) %>%
...
##
## To recap: here the most common ways to prevent overfitting in
## neural networks:
...
##  > Add dropout.
##
## And two important approaches not covered in this guide are data
## augmentation and batch normalization.
##
rm(imdb,train_data,train_labels,test_data,test_labels,compare_cx)
if(FALSE) {
    load(file="../../Data/imdb.rda")
    ##
    c(train_data, train_labels) %<-% imdb$train
